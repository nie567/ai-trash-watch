...
<body>
    ...
    <div class="main-content">
                    ...
                    <tbody>
                        <c:forEach items="${detail.detections}" var="d">
                            <tr>
                                <td>${d.className}</td>
                                <td>${d.confidence}%</td>
                                <td><span class="badge ${d.mappedCategory == '可回收物' ? 'category-recyclable' : d.mappedCategory == '厨余垃圾' ? 'category-kitchen' : d.mappedCategory == '有害垃圾' ? 'category-hazardous' : d.mappedCategory == '其他垃圾' ? 'category-other' : 'category-mixed'}">${d.mappedCategory}</span></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
        ...
        <!-- 违规信息 -->
        <c:if test="${not empty detail.violation}">
            <div class="card">
                <h2 class="card-title">违规信息</h2>
                <div class="info-grid">
                    <div class="info-item"><span class="info-label">违规类型：</span><span class="info-value">${detail.violation.violationType}</span></div>
                    <div class="info-item"><span class="info-label">违规级别：</span><span class="info-value"><span class="badge ${detail.violation.level == 'LOW' ? 'badge-pending' : detail.violation.level == 'MEDIUM' ? 'badge-submitted' : 'badge-rejected'}">${detail.violation.level}</span></span></div>
                    <div class="info-item"><span class="info-label">描述：</span><span class="info-value">${detail.violation.description}</span></div>
                    <div class="info-item"><span class="info-label">状态：</span><span class="info-value"><span class="badge ${detail.violation.status == 'PENDING' ? 'badge-pending' : detail.violation.status == 'RECTIFIED' ? 'badge-rectified' : 'badge-ignored'}">${detail.violation.status}</span></span></div>
                </div>
            </div>
        </c:if>
        ...
        <c:if test="${not empty detail.rectification}">
            <div class="card">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item"><span class="info-label">整改期限：</span><span class="info-value">${detail.rectification.deadline}</span></div>
                    <div class="info-item"><span class="info-label">状态：</span><span class="info-value"><span class="badge badge-${detail.rectification.status eq 'PENDING' ? 'pending' : detail.rectification.status eq 'SUBMITTED' ? 'submitted' : detail.rectification.status eq 'APPROVED' ? 'approved' : 'rejected'}">${detail.rectification.status}</span></span></div>
                </div>
